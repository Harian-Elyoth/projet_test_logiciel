...
while is_valid == False:
	print("press n pour nouveau client")
	print("press c pour client existe")
	...
	if(commend_insc == "n"):
		...
		print("adminStatus")
		the_adminStatus = input("> ")

		query = {'firstName':[''],'lastName':[''],'username': [''], 'password': [''],'adminStatus': ['']} 
		query['firstName'] = the_firstname
		query['lastName'] = the_lastname
		query['username'] = the_username
		query['password'] = the_password
		query['adminStatus'] = the_adminStatus

		myClientsSql.insert("#/User",query)
		is_valid = True
		...
		dataBase_mot = myClientsSql.select('#/User/password/username/"%s"' % (client_nb))
		# print(dataBase_mot[0][0])

		if(dataBase_mot == client_mot):
			is_valid = True
			# port_nb = 60001
		else:
			print("réesayer le nom et le mot")
